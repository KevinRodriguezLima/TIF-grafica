...
def train_content_classifier(
    ...
    if torch is None:
        raise ImportError(
            "Instala torch y torchvision para entrenar el clasificador"
        )
    ...
    if not dataset.samples:
        raise ValueError("El dataset de contenido no tiene imágenes")
    ...
    model = ContentClassifierNetwork(
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("models/content_classifier.pt"),
    )
    ...
    result = train_content_classifier(
        args.dataset,
        args.output,
        args.epochs,
        args.batch_size,
